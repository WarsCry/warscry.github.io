  ...
  class XenoPinball2D {
    ...
    configure(index, profile) {
      ...
      this.walls = [
        ...
        [625,305,625,590],[58,725,205,918],[620,750,555,918],[122,700,185,615],[600,690,545,620],
        [205,918,252,958],[555,918,508,958],
      ].map((s, i) => ({ ax:s[0], ay:s[1], bx:s[2], by:s[3], width:i < 5 ? 8 : 6 }));
      this.slings = [{ x:235,y:800,r:25,index:20,last:0 },{ x:525,y:800,r:25,index:21,last:0 }];
      this.upperWalls = [
    ...
    addMultiball() {
      if (!this.active) return;
      if (this.balls.filter((ball) => ball.alive).length >= 2) return;
      this.balls.push(this.makeLiveBall(380,245,(Math.random() < .5 ? -1 : 1) * 310,-180));
    ...
    queueExtraBall() {
      ...
      this.extraQueued = true;
      return this.loadQueuedExtra();
    }

    loadQueuedExtra() {
      if (!this.extraQueued || this.balls.filter((ball) => ball.alive).length >= 2) return false;
      this.extraQueued = false;
      this.serveBall();
      return true;
    }
    ...
    step(dt, now) {
      ...
      for (const ball of [...this.balls]) {
        ...
        if(ball.layer===0&&ball.layerCooldown===0&&ball.entered&&ball.y<145&&ball.x>225&&ball.x<535){
          ball.layer=1;ball.layerTime=0;this.activeLayer=1;ball.x=380;ball.y=940;ball.vx*=.3;ball.vy=-Math.max(460,Math.abs(ball.vy)*.68);ball.trail=[];ball.stuckFor=0;ball.stuckX=ball.x;ball.stuckY=ball.y;
          ...
          ball.layer=0;ball.layerTime=0;ball.layerCooldown=4;this.activeLayer=0;ball.x=380;ball.y=165;ball.vx*=.45;ball.vy=Math.max(260,Math.abs(ball.vy)*.48);ball.trail=[];ball.stuckFor=0;ball.stuckX=ball.x;ball.stuckY=ball.y;
        ...
        const anchorTravel=Math.hypot(ball.x-ball.stuckX,ball.y-ball.stuckY);
        if(anchorTravel>34){ball.stuckX=ball.x;ball.stuckY=ball.y;ball.stuckFor=0}else ball.stuckFor+=dt;
        if(ball.stuckFor>1.2){
          const rightChute=ball.layer===0&&ball.x>555&&ball.y>555&&ball.y<880;
          if(rightChute){ball.x=Math.min(ball.x,555);ball.vx=-240;ball.vy=180}
          else{ball.vx=clamp((380-ball.x)*.6,-220,220)+(ball.x<380?70:-70);ball.vy=ball.y>850?-260:190}
          ball.stuckX=ball.x;ball.stuckY=ball.y;ball.stuckFor=0;this.callbacks.onUnstuck?.()
        }
    ...
    drain(ball){
      if(!ball.alive)return;ball.alive=false;this.burst(ball.x,1065,this.theme.hot,20,1.4);this.balls=this.balls.filter(item=>item.alive);this.loadQueuedExtra();this.callbacks.onDrain?.(this.balls.length);
    }
