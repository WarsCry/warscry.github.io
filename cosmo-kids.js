...
let stars=0,currentGame='',soundOn=true,audioContext,praiseIndex=0,theme='cosmos',language='fr',lastMemorySignature='',storyStep=0,storyRewarded=false,storyVoiceToken=0;
...
function speak(text,calm=false,onDone){
  if(!soundOn||!('speechSynthesis'in window)){if(onDone)setTimeout(onDone,Math.min(7000,Math.max(3500,text.length*70)));return}
