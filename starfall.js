...
const WALK_SPEED=.5,BOOST_SPEED=WALK_SPEED*1.22,BASE_FOV=.92,AIM_FOV=.58,BOOST_FOV=1.03,HULL_LIMIT=34;
...
async function loadDeckThreeSpecimens(){
  ...
  try{
    ...
    const silver=pbr('imported containment glass','#9fc1c0',.65,.2,'#071b19'),capMetal=pbr('brushed containment caps','#8da2a3',.92,.2),sealGlow=material('containment seal light','#5bf5cd','#20bb91'),alienSkin=material('imported specimen blue-green skin','#1f8f77','#093f35');silver.alpha=.1;silver.transparencyMode=BABYLON.PBRMaterial.PBRMATERIAL_ALPHABLEND;silver.backFaceCulling=false;silver.disableDepthWrite=true;alienSkin.specularColor=C.FromHexString('#a5fff0');alienSkin.specularPower=112;
    for(const slot of deckThreeSpecimenSlots){
      ...
      alienRoot.getChildMeshes().forEach(mesh=>{mesh.material=alienSkin;mesh.useVertexColors=false;mesh.checkCollisions=false;mesh.isPickable=false;mesh.renderingGroupId=0;shadowGenerator.addShadowCaster(mesh)});
...
function attachEnemyArtwork(enemy){
  ...
  const legacyMeshes=enemy.getChildMeshes(),instance=enemyModelContainer.instantiateModelsToScene(name=>`${enemy.name} artwork ${name}`,false),artwork=instance.rootNodes[0],boss=enemy.metadata.boss,robotMaterial=material(`${enemy.name} imported armour`,boss?'#a92f47':'#a7b7ba',boss?'#3f0b18':'#203539');robotMaterial.specularColor=C.FromHexString(boss?'#ffb2bc':'#ecffff');robotMaterial.specularPower=150;
  ...
  for(const mesh of artwork.getChildMeshes()){mesh.material=robotMaterial;mesh.useVertexColors=false;mesh.metadata={enemy};mesh.isPickable=true;mesh.receiveShadows=true;shadowGenerator.addShadowCaster(mesh)}
  const chestMaterial=material(`${enemy.name} navy chest plate`,boss?'#4e101d':'#172d3b',boss?'#170409':'#06141b'),screenColor=boss?'#ff465f':'#22eaff',screenMaterial=material(`${enemy.name} chest screens`,screenColor,screenColor),shoulderMaterial=material(`${enemy.name} shoulder accents`,boss?'#ff7d3d':'#285f78',boss?'#8b250e':'#0b3547');chestMaterial.disableLighting=screenMaterial.disableLighting=shoulderMaterial.disableLighting=true;for(const z of [-.47,.47]){const chest=BABYLON.MeshBuilder.CreateBox(`${enemy.name} coloured chest plate`,{width:.84,height:.52,depth:.04},scene);chest.parent=artwork;chest.position=new V(0,.28,z);chest.material=chestMaterial;chest.isPickable=false;for(const x of [-.2,.2]){const screen=BABYLON.MeshBuilder.CreateBox(`${enemy.name} cyan chest screen`,{width:.28,height:.18,depth:.045},scene);screen.parent=artwork;screen.position=new V(x,.3,z+(z>0?.035:-.035));screen.material=screenMaterial;screen.isPickable=false}}for(const x of [-.69,.69]){const shoulder=BABYLON.MeshBuilder.CreateSphere(`${enemy.name} coloured shoulder cap`,{diameter:.34,segments:10},scene);shoulder.parent=artwork;shoulder.position=new V(x,.48,0);shoulder.scaling=new V(1,.75,1);shoulder.material=shoulderMaterial;shoulder.isPickable=false}
}
function attachPlayerWeapon(){
  if(!weaponModelContainer||playerWeaponRoot)return;
  const instance=weaponModelContainer.instantiateModelsToScene(name=>`sentinel rifle artwork ${name}`,false),root=instance.rootNodes[0],gunMaterial=material('sentinel rifle deep blue alloy','#1b4c8f','#071a3b');gunMaterial.specularColor=C.FromHexString('#79a8df');gunMaterial.specularPower=125;root.parent=camera;root.position=new V(.5,-.72,1);root.rotation=new V(-.4,Math.PI-.25,-.12);root.scaling.setAll(1.32);root.getChildMeshes().forEach(mesh=>{mesh.material=gunMaterial;mesh.useVertexColors=false;mesh.isPickable=false;mesh.renderingGroupId=2;mesh.alwaysSelectAsActiveMesh=true});const cyanRail=material('temporary cyan gun rail','#32ffe3','#32ffe3'),violetRail=material('temporary violet gun rail','#a864ff','#a864ff');cyanRail.disableLighting=violetRail.disableLighting=true;for(const [x,railMaterial] of [[-.11,cyanRail],[.11,violetRail]]){const rail=BABYLON.MeshBuilder.CreateBox('coloured rifle top rail',{width:.065,height:.045,depth:1.12},scene);rail.parent=root;rail.position=new V(x,.6,.04);rail.material=railMaterial;rail.isPickable=false;rail.renderingGroupId=2;rail.alwaysSelectAsActiveMesh=true}
