...
async function loadDeckThreeSpecimens(){
  ...
  try{
    ...
    const silver=pbr('imported containment glass','#9fc1c0',.65,.2,'#071b19'),capMetal=pbr('brushed containment caps','#8da2a3',.92,.2),sealGlow=material('containment seal light','#5bf5cd','#20bb91'),alienSkin=pbr('imported specimen skin','#9cc7bf',.12,.4,'#082a27');silver.alpha=.1;silver.transparencyMode=BABYLON.PBRMaterial.PBRMATERIAL_ALPHABLEND;silver.backFaceCulling=false;silver.disableDepthWrite=true;alienSkin.emissiveIntensity=.36;
    for(const slot of deckThreeSpecimenSlots){
      ...
      const alienHighlight=C.FromHexString('#6ca99f'),alienBody=C.FromHexString('#386b65'),alienShadow=C.FromHexString('#183f3c');alienRoot.getChildMeshes().forEach(mesh=>{mesh.material=alienSkin;paintMesh(mesh,(x,y,z)=>y>.38?alienHighlight:y<-.34||z<-.5?alienShadow:alienBody);mesh.checkCollisions=false;mesh.isPickable=false;mesh.renderingGroupId=0;shadowGenerator.addShadowCaster(mesh)});
...
function attachEnemyArtwork(enemy){
  ...
  const legacyMeshes=enemy.getChildMeshes(),instance=enemyModelContainer.instantiateModelsToScene(name=>`${enemy.name} artwork ${name}`,false),artwork=instance.rootNodes[0],boss=enemy.metadata.boss,dark=C.FromHexString(boss?'#3d1720':'#26363a'),steel=C.FromHexString(boss?'#a23b4c':'#778b8f'),silver=C.FromHexString(boss?'#da7480':'#b4c3c4'),robotMaterial=pbr(`${enemy.name} imported armour`,'#dce5e3',.9,.24,boss?'#26070e':'#030b0d');
  ...
  for(const mesh of artwork.getChildMeshes()){mesh.material=robotMaterial;paintMesh(mesh,(x,y,z)=>y<-.55||Math.abs(y)<.12&&Math.abs(x)<.22?dark:y>.48||Math.abs(x)>.55?silver:steel);mesh.metadata={enemy};mesh.isPickable=true;mesh.receiveShadows=true;shadowGenerator.addShadowCaster(mesh)}
  const screenColor=boss?'#ff465f':'#20dff1',screenMaterial=material(`${enemy.name} chest screens`,screenColor,screenColor);for(const z of [-.43,.43])for(const x of [-.2,.2]){const screen=BABYLON.MeshBuilder.CreateBox(`${enemy.name} cyan chest screen`,{width:.25,height:.16,depth:.035},scene);screen.parent=artwork;screen.position=new V(x,.28,z);screen.material=screenMaterial;screen.isPickable=false}
}
function attachPlayerWeapon(){
  if(!weaponModelContainer||playerWeaponRoot)return;
  const instance=weaponModelContainer.instantiateModelsToScene(name=>`sentinel rifle artwork ${name}`,false),root=instance.rootNodes[0],gunMaterial=pbr('sentinel rifle blue-black alloy','#8a9ab1',.82,.25,'#020713'),alloy=C.FromHexString('#18243a'),edge=C.FromHexString('#344863'),grip=C.FromHexString('#0b1220');gunMaterial.emissiveIntensity=.62;root.parent=camera;root.position=new V(.5,-.72,1);root.rotation=new V(-.4,Math.PI-.25,-.12);root.scaling.setAll(1.32);root.getChildMeshes().forEach(mesh=>{mesh.material=gunMaterial;paintMesh(mesh,(x,y,z)=>y<-.18&&z<.5?grip:z>.42||Math.abs(x)<.075&&y>.04?edge:alloy);mesh.isPickable=false;mesh.renderingGroupId=2;mesh.alwaysSelectAsActiveMesh=true});
