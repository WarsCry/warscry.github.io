...
const audioState={ac:null,master:null,music:null,enemies:null,running:false,timer:null,drones:[],beat:0,danger:.2,lastVoice:0,lastStep:0};
...
function ensureAudio(){
  try{
    ...
    if(!audioState.master){audioState.master=ac.createGain();audioState.master.gain.value=.72;audioState.master.connect(ac.destination);audioState.music=ac.createGain();audioState.music.gain.value=.0001;audioState.music.connect(audioState.master);audioState.enemies=ac.createGain();audioState.enemies.gain.value=.8;audioState.enemies.connect(audioState.master)}
    return ac;
  }catch{return null}
}
