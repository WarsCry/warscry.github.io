...
function attachEnemyArtwork(enemy){
  ...
  const legacyMeshes=enemy.getChildMeshes(),instance=enemyModelContainer.instantiateModelsToScene(name=>`${enemy.name} artwork ${name}`,false),artwork=new BABYLON.TransformNode(`${enemy.name} robot artwork`,scene),boss=enemy.metadata.boss;
  for(const root of instance.rootNodes)root.parent=artwork;
  ...
  for(const mesh of artwork.getChildMeshes()){mesh.metadata={enemy};mesh.isPickable=true;mesh.receiveShadows=true;shadowGenerator.addShadowCaster(mesh);if(boss){mesh.renderOutline=true;mesh.outlineColor=C.FromHexString('#ff465f');mesh.outlineWidth=.035}}
}
