...
async function loadDeckThreeSpecimens(){
  ...
  try{
    ...
    const silver=pbr('imported containment glass','#9fc1c0',.65,.2,'#071b19'),capMetal=pbr('brushed containment caps','#8da2a3',.92,.2),sealGlow=material('containment seal light','#5bf5cd','#20bb91'),alienSkin=material('imported specimen blue-green skin','#2d5f59','#071c19');silver.alpha=.1;silver.transparencyMode=BABYLON.PBRMaterial.PBRMATERIAL_ALPHABLEND;silver.backFaceCulling=false;silver.disableDepthWrite=true;alienSkin.specularColor=C.FromHexString('#8ed5cb');alienSkin.specularPower=112;
...
function attachEnemyArtwork(enemy){
  ...
  const screenColor=boss?'#ff465f':'#22eaff',screenMaterial=material(`${enemy.name} chest screens`,screenColor,screenColor);for(const z of [-.465,.465])for(const x of [-.2,.2]){const screen=BABYLON.MeshBuilder.CreateBox(`${enemy.name} cyan chest screen`,{width:.29,height:.2,depth:.045},scene);screen.parent=artwork;screen.position=new V(x,.3,z);screen.material=screenMaterial;screen.isPickable=false}
}
function attachPlayerWeapon(){
  if(!weaponModelContainer||playerWeaponRoot)return;
  const instance=weaponModelContainer.instantiateModelsToScene(name=>`sentinel rifle artwork ${name}`,false),root=instance.rootNodes[0],gunMaterial=material('sentinel rifle blue-black alloy','#121e32','#02050c');gunMaterial.specularColor=C.FromHexString('#405879');gunMaterial.specularPower=125;root.parent=camera;root.position=new V(.5,-.72,1);root.rotation=new V(-.4,Math.PI-.25,-.12);root.scaling.setAll(1.32);root.getChildMeshes().forEach(mesh=>{mesh.material=gunMaterial;mesh.useVertexColors=false;mesh.isPickable=false;mesh.renderingGroupId=2;mesh.alwaysSelectAsActiveMesh=true});
