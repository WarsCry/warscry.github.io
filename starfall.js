...
async function loadDeckThreeSpecimens(){
  ...
  try{
    ...
    for(const slot of deckThreeSpecimenSlots){
      ...
      alienRoot.getChildMeshes().forEach(mesh=>{mesh.material=alienSkin;mesh.checkCollisions=false;mesh.isPickable=false;mesh.renderingGroupId=0;shadowGenerator.addShadowCaster(mesh)});
...
function attachEnemyArtwork(enemy){
  ...
  const legacyMeshes=enemy.getChildMeshes(),instance=enemyModelContainer.instantiateModelsToScene(name=>`${enemy.name} artwork ${name}`,false),artwork=instance.rootNodes[0],boss=enemy.metadata.boss,tone=boss?'#8d3138':['#70808c','#755c78','#4d8170','#56729a'][Math.min(3,currentLevel-1)],robotMaterial=pbr(`${enemy.name} imported armour`,tone,.9,.28,boss?'#351016':'#08191c');
  ...
  for(const mesh of artwork.getChildMeshes()){mesh.material=robotMaterial;mesh.metadata={enemy};mesh.isPickable=true;mesh.receiveShadows=true;shadowGenerator.addShadowCaster(mesh)}
}
function attachPlayerWeapon(){
  if(!weaponModelContainer||playerWeaponRoot)return;
  const instance=weaponModelContainer.instantiateModelsToScene(name=>`sentinel rifle artwork ${name}`,false),root=instance.rootNodes[0],gunMaterial=pbr('sentinel rifle dark alloy','#18243a',.9,.27,'#020812');root.parent=camera;root.position=new V(.5,-.72,1);root.rotation=new V(-.4,Math.PI-.25,-.12);root.scaling.setAll(1.32);root.getChildMeshes().forEach(mesh=>{mesh.material=gunMaterial;mesh.isPickable=false;mesh.renderingGroupId=2;mesh.alwaysSelectAsActiveMesh=true});playerWeaponRoot=root;scene.setRenderingAutoClearDepthStencil(2,true,true,true);document.body.classList.add('model-weapon');
}
