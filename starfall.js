...
let scene,camera,core,coreLight,ambientLight,shadowGenerator,enemies=[],projectiles=[],specimens=[],deckThreeSpecimenSlots=[],deckThreeModelContainers=[],enemyModelContainer=null,weaponModelContainer=null,playerWeaponRoot=null,weaponRecoil=0,interactiveNeons=[],brokenNeons=new Set(),started=false,health=100,ammo=40,kills=0,lastShot=0,aiming=false,sprinting=false,moveVector={x:0,y:0},turnDirection=0,currentLevel=1,pendingLevel=1,levelStartedAt=0;
...
let selectedDifficulty=DIFFICULTIES[new URLSearchParams(location.search).get('difficulty')]?new URLSearchParams(location.search).get('difficulty'):'novice';
...
function artworkMaterial(name,url){const m=material(name,'#c9d7d8'),texture=new BABYLON.Texture(url,scene);texture.anisotropicFilteringLevel=16;m.diffuseTexture=texture;m.emissiveTexture=texture;m.emissiveColor=new C(.18,.18,.18);m.specularColor=new C(.08,.11,.12);m.backFaceCulling=false;return m}
function paintMesh(mesh,painter){const positions=mesh.getVerticesData(BABYLON.VertexBuffer.PositionKind);if(!positions)return;const colors=[];for(let i=0;i<positions.length;i+=3){const color=painter(positions[i],positions[i+1],positions[i+2]);colors.push(color.r,color.g,color.b,1)}mesh.setVerticesData(BABYLON.VertexBuffer.ColorKind,colors,true);mesh.useVertexColors=true;mesh.hasVertexAlpha=false}
...
function createDeckThreeSpecimenFallback(slot){
  const glass=material(`reserve nutrient glass ${slot.index}`,'#173c42','#0a6170',.34),seal=material(`reserve chamber seal ${slot.index}`,'#657778','#74ffe0'),skin=material(`reserve alien skin ${slot.index}`,'#7fe4c1','#143d35'),belly=material(`reserve alien belly ${slot.index}`,'#b7ffe1','#17483c'),eyes=material(`reserve alien eyes ${slot.index}`,'#a9fff0','#5affdf'),cord=material(`reserve nutrient cord ${slot.index}`,'#73c8c4','#287e76');glass.backFaceCulling=false;glass.needDepthPrePass=true;
  const tank=onDeck(3,BABYLON.MeshBuilder.CreateCylinder('reserve growth tank',{height:4.8,diameter:2.9,tessellation:24},scene)),fallbackNodes=[tank];tank.position=new V(slot.x,2.5,slot.z);tank.material=glass;tank.metadata={glassTank:true,cracks:0};tank.isPickable=true;
  for(const y of [.3,4.7]){const cap=onDeck(3,BABYLON.MeshBuilder.CreateCylinder('reserve chamber metal cap',{height:.34,diameter:3.15,tessellation:28},scene));cap.position=new V(slot.x,y,slot.z);cap.material=seal;cap.isPickable=false;fallbackNodes.push(cap)}
  fallbackNodes.push(createBabyAlienSpecimen(slot.x,slot.z,slot.index,slot.angle,skin,belly,eyes,cord));fallbackNodes.forEach(node=>node.setEnabled(currentLevel===3));
}
async function loadDeckThreeSpecimens(){
  if(!BABYLON.SceneLoader?.LoadAssetContainerAsync){deckThreeSpecimenSlots.forEach(createDeckThreeSpecimenFallback);return}
  try{
    const [cylinderContainer,alienContainer]=await Promise.all([
      BABYLON.SceneLoader.LoadAssetContainerAsync('assets/starfall/models/','specimen-cylinder.glb',scene),
      BABYLON.SceneLoader.LoadAssetContainerAsync('assets/starfall/models/','specimen-alien.glb',scene)
    ]);deckThreeModelContainers.push(cylinderContainer,alienContainer);
    const silver=pbr('imported containment glass','#9fc1c0',.65,.2,'#071b19'),capMetal=pbr('brushed containment caps','#8da2a3',.92,.2),sealGlow=material('containment seal light','#5bf5cd','#20bb91'),alienSkin=pbr('imported specimen green','#65c696',.18,.48,'#174c39');silver.alpha=.1;silver.transparencyMode=BABYLON.PBRMaterial.PBRMATERIAL_ALPHABLEND;silver.backFaceCulling=false;silver.disableDepthWrite=true;alienSkin.emissiveIntensity=.42;
    for(const slot of deckThreeSpecimenSlots){
      const cylinderInstance=cylinderContainer.instantiateModelsToScene(name=>`deck 09 cylinder ${slot.index} ${name}`,false),cylinderRoot=cylinderInstance.rootNodes[0];
      cylinderRoot.position=new V(slot.x,2.3,slot.z);cylinderRoot.rotation.y=slot.angle+Math.PI;cylinderRoot.scaling=new V(1.53,2.68,1.53);onDeck(3,cylinderRoot);cylinderRoot.setEnabled(currentLevel===3);
      cylinderRoot.getChildMeshes().forEach(mesh=>{mesh.material=silver;mesh.checkCollisions=false;mesh.isPickable=true;mesh.renderingGroupId=1;mesh.metadata={glassTank:true,cracks:0,importedSpecimenChamber:true}});
      for(const y of [.28,4.96]){const cap=onDeck(3,BABYLON.MeshBuilder.CreateCylinder('brushed specimen chamber cap',{height:.4,diameter:3.28,tessellation:36},scene));cap.position=new V(slot.x,y,slot.z);cap.material=capMetal;cap.isPickable=false;cap.setEnabled(currentLevel===3);const seal=onDeck(3,BABYLON.MeshBuilder.CreateTorus('specimen chamber luminous seal',{diameter:3.12,thickness:.105,tessellation:36},scene));seal.position=new V(slot.x,y+(y<1?.23:-.23),slot.z);seal.material=sealGlow;seal.isPickable=false;seal.setEnabled(currentLevel===3)}
      const alienInstance=alienContainer.instantiateModelsToScene(name=>`deck 09 alien ${slot.index} ${name}`,false),alienRoot=alienInstance.rootNodes[0];
      alienRoot.position=new V(slot.x,2.45,slot.z);alienRoot.rotation.y=slot.angle+Math.PI;alienRoot.scaling=new V(.72,1.24,.72);onDeck(3,alienRoot);alienRoot.setEnabled(currentLevel===3);
      const alienMint=C.FromHexString('#72d6a7'),alienLime=C.FromHexString('#c4ef76'),alienDeep=C.FromHexString('#174e48');alienRoot.getChildMeshes().forEach(mesh=>{mesh.material=alienSkin;paintMesh(mesh,(x,y,z)=>y>.34?alienMint:y<-.32?alienDeep:Math.abs(x)>.46?alienLime:alienMint);mesh.checkCollisions=false;mesh.isPickable=false;mesh.renderingGroupId=0;shadowGenerator.addShadowCaster(mesh)});
      specimens.push({root:alienRoot,baseY:2.45,baseRotation:slot.angle+Math.PI,phase:slot.index*.82,bubbles:[]});
    }
  }catch(error){console.warn('Deck 09 specimen models could not load; using lightweight reserve chambers.',error);deckThreeSpecimenSlots.forEach(createDeckThreeSpecimenFallback)}
}
function attachEnemyArtwork(enemy){
  if(!enemyModelContainer||enemy.metadata.parts.artwork)return;
  const legacyMeshes=enemy.getChildMeshes(),instance=enemyModelContainer.instantiateModelsToScene(name=>`${enemy.name} artwork ${name}`,false),artwork=instance.rootNodes[0],boss=enemy.metadata.boss,palettes=boss?['#5b1528','#ef476f','#ffb14a']:['#152331','#2d7180','#59e1d0','#203253','#773d81','#ff71b6','#17342c','#43866c','#b9ed58','#1a2944','#426ca5','#70cfff'].slice((Math.min(4,currentLevel)-1)*3,Math.min(4,currentLevel)*3),dark=C.FromHexString(palettes[0]),armour=C.FromHexString(palettes[1]),accent=C.FromHexString(palettes[2]),robotMaterial=pbr(`${enemy.name} imported armour`,'#dce5e8',.76,.31,boss?'#26070e':'#02080c');
  artwork.parent=enemy;artwork.position=new V(0,2.05,0);artwork.rotation.y=Math.PI;artwork.scaling.setAll(2.05);enemy.metadata.parts.artwork=artwork;
  for(const mesh of legacyMeshes){if(mesh===enemy.metadata.parts.healthFill||mesh.name==='enemy health back'||mesh===enemy.metadata.parts.muzzle)continue;mesh.visibility=0;mesh.isPickable=false}
  for(const mesh of artwork.getChildMeshes()){mesh.material=robotMaterial;paintMesh(mesh,(x,y,z)=>y<-.52?dark:y>.5||Math.abs(x)>.52?accent:Math.abs(x)<.37&&y>-.28?armour:dark);mesh.metadata={enemy};mesh.isPickable=true;mesh.receiveShadows=true;shadowGenerator.addShadowCaster(mesh)}
  const glowMaterial=material(`${enemy.name} chest reactor`,palettes[2],palettes[2]);for(const z of [-.43,.43]){const reactor=BABYLON.MeshBuilder.CreateSphere(`${enemy.name} chest reactor light`,{diameter:.16,segments:10},scene);reactor.parent=artwork;reactor.position=new V(0,.25,z);reactor.scaling=new V(1.25,.7,.32);reactor.material=glowMaterial;reactor.isPickable=false}
}
function attachPlayerWeapon(){
  if(!weaponModelContainer||playerWeaponRoot)return;
  const instance=weaponModelContainer.instantiateModelsToScene(name=>`sentinel rifle artwork ${name}`,false),root=instance.rootNodes[0],gunMaterial=pbr('sentinel rifle dark alloy','#8cb4cc',.78,.28,'#0a2637'),alloy=C.FromHexString('#233754'),violet=C.FromHexString('#9b68e5'),plasma=C.FromHexString('#76ffe8'),grip=C.FromHexString('#863f62');gunMaterial.emissiveIntensity=1.25;root.parent=camera;root.position=new V(.5,-.72,1);root.rotation=new V(-.4,Math.PI-.25,-.12);root.scaling.setAll(1.32);root.getChildMeshes().forEach(mesh=>{mesh.material=gunMaterial;paintMesh(mesh,(x,y,z)=>y<-.18&&z<.5?grip:Math.abs(z)<.16?plasma:z>.5?violet:alloy);mesh.isPickable=false;mesh.renderingGroupId=2;mesh.alwaysSelectAsActiveMesh=true});
  const plasmaMaterial=material('rifle cyan energy rail','#63ffe1','#35ffd8'),violetMaterial=material('rifle violet energy cell','#8759d9','#5b28bd');for(const [name,size,position,accentMaterial] of [['cyan plasma rail',{width:.065,height:.05,depth:.72},new V(.02,.18,.12),plasmaMaterial],['violet plasma rail',{width:.055,height:.045,depth:.56},new V(-.13,.11,.08),violetMaterial],['rifle energy cell',{width:.23,height:.1,depth:.22},new V(.02,.08,-.28),violetMaterial]]){const accent=BABYLON.MeshBuilder.CreateBox(name,size,scene);accent.parent=root;accent.position=position;accent.material=accentMaterial;accent.isPickable=false;accent.renderingGroupId=2;accent.alwaysSelectAsActiveMesh=true}
  playerWeaponRoot=root;scene.setRenderingAutoClearDepthStencil(2,true,true,true);document.body.classList.add('model-weapon');
}
async function loadCombatArtwork(){
  if(!BABYLON.SceneLoader?.LoadAssetContainerAsync)return;
  const [enemyResult,weaponResult]=await Promise.allSettled([
    BABYLON.SceneLoader.LoadAssetContainerAsync('assets/starfall/models/','dominion-robot.glb',scene),
    BABYLON.SceneLoader.LoadAssetContainerAsync('assets/starfall/models/','sentinel-rifle.glb',scene)
  ]);
  if(enemyResult.status==='fulfilled'){enemyModelContainer=enemyResult.value;enemies.forEach(attachEnemyArtwork)}else console.warn('The imported Dominion robot could not load.',enemyResult.reason);
  if(weaponResult.status==='fulfilled'){weaponModelContainer=weaponResult.value;attachPlayerWeapon()}else console.warn('The imported sentinel rifle could not load.',weaponResult.reason);
}
function updatePlayerWeapon(dt,t){if(!playerWeaponRoot)return;weaponRecoil=Math.max(0,weaponRecoil-dt*8);const aim=aiming?1:0,compact=innerWidth<=600,tablet=!compact&&innerWidth<=900,baseX=compact?.34:tablet?.4:.5,baseY=compact?-.6:tablet?-.72:-.72,baseScale=compact?.78:tablet?.95:1.32,bob=started&&!aiming?Math.sin(t*5.2)*.014:0;playerWeaponRoot.position.x=aim?.035:baseX+Math.sin(t*2.6)*.014;playerWeaponRoot.position.y=baseY+aim*.18+bob+(sprinting?Math.abs(Math.sin(t*8))*.04:0);playerWeaponRoot.position.z=1+aim*.2-weaponRecoil*.15;playerWeaponRoot.rotation.x=-.4+aim*.17+weaponRecoil*.075+(sprinting?.08:0);playerWeaponRoot.rotation.y=Math.PI-.25+aim*.23;playerWeaponRoot.rotation.z=-.12+(sprinting?Math.sin(t*8)*.025:0);playerWeaponRoot.scaling.setAll(aim?baseScale*.78:baseScale)}
...
function buildScene(){
  ...
  const spaceshipArtwork=artworkMaterial('new spaceship wall artwork','assets/starfall/spaceship-wall-v2.webp'),coreArtwork=artworkMaterial('new core observatory artwork','assets/starfall/core-observatory-v2.webp'),deckArtwork={1:coreArtwork,2:spaceshipArtwork,3:spaceshipArtwork,4:coreArtwork,5:spaceshipArtwork};
  for(const level of [1,2,3,4,5])for(const angle of [0,Math.PI/2,Math.PI,Math.PI*1.5]){const panel=onDeck(level,BABYLON.MeshBuilder.CreatePlane(`deck ${level} detailed spaceship backdrop`,{width:16,height:8,sideOrientation:BABYLON.Mesh.DOUBLESIDE},scene)),radius=34;panel.position=new V(Math.sin(angle)*radius,4.45,Math.cos(angle)*radius);panel.rotation.y=angle;panel.material=deckArtwork[level];panel.isPickable=false;panel.receiveShadows=false}
  ...
  const bioAlloy=texturedMaterial('hydroponics living alloy','assets/textures/starfall-living-machinery-v2.webp',.72,1.05),bioDark=pbr('hydroponics machinery','#071814',.72,.34),bioGlow=material('growth energy','#56ffc2','#19a86f'),specimenCollisionMaterial=material('invisible specimen collision','#000000',null,0);bioAlloy.diffuseColor=C.FromHexString('#6b8a73');specimenCollisionMaterial.disableColorWrite=true;
  for(let i=0;i<12;i++){
    const a=i*Math.PI*2/12,r=i%2?22:16,x=Math.sin(a)*r,z=Math.cos(a)*r;
    if(i%2){const collision=onDeck(3,BABYLON.MeshBuilder.CreateCylinder('specimen chamber collision',{height:5.35,diameter:3.15,tessellation:16},scene));collision.position=new V(x,2.68,z);collision.material=specimenCollisionMaterial;collision.visibility=0;collision.checkCollisions=true;collision.isPickable=false;deckThreeSpecimenSlots.push({x,z,index:i,angle:a})}
    else{const tank=onDeck(3,BABYLON.MeshBuilder.CreateCylinder('rounded growth tank',{height:4.6,diameter:2.8,tessellation:24},scene));tank.position=new V(x,2.3,z);tank.material=bioAlloy;tank.checkCollisions=true;const crown=onDeck(3,BABYLON.MeshBuilder.CreateSphere('growth chamber crown',{diameter:2.25,segments:18},scene));crown.position=new V(x,4.25,z);crown.scaling.y=.55;crown.material=bioGlow;crown.isPickable=false;const root=onDeck(3,BABYLON.MeshBuilder.CreateTorus('growth tank base',{diameter:3.1,thickness:.22,tessellation:28},scene));root.position=new V(x,.28,z);root.material=bioGlow;root.isPickable=false}
  ...
  scene.onBeforeRenderObservable.add(()=>{const t=performance.now()*.001,dt=scene.getEngine().getDeltaTime()/1000;if(core){core.scaling.setAll(1+Math.sin(t*2.4)*.045);core.rotation.y=t*.25;coreHalo.rotation.z=t*.3}for(const specimen of specimens){specimen.root.position.y=specimen.baseY+Math.sin(t*1.25+specimen.phase)*.075;specimen.root.rotation.y=specimen.baseRotation+Math.sin(t*.7+specimen.phase)*.12;specimen.root.rotation.z=Math.sin(t*.9+specimen.phase)*.035;specimen.bubbles.forEach((bubble,index)=>bubble.position.y=-1.28+((t*.24+specimen.phase*.15+index*.42)%2.4))}updatePlayerWeapon(dt,t);if(started)update(dt,t)});
  return scene;
}
...
buildScene();loadDeckThreeSpecimens();reset(1);loadCombatArtwork();engine.runRenderLoop(()=>scene.render());addEventListener('resize',()=>engine.resize());
...
document.querySelector(`[data-difficulty="${selectedDifficulty}"]`)?.click();
document.querySelector('#start').onclick=()=>{reset(pendingLevel);started=true;startCombatAudio();menu.classList.add('hidden');show(`${DIFFICULTIES[selectedDifficulty].label} · SHIELD ${LEVELS[currentLevel].grace.toFixed(1)} SECONDS`);try{const lock=canvas.requestPointerLock?.();lock?.catch?.(()=>{})}catch{}setTimeout(()=>{if(started)enemyVoice(enemies[0],'start',true)},1100)};
